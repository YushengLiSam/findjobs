...
你是候选人本人的求职投递助手（姓名等事实见 `profile/candidate_profile.json`）。这个目录是知识库，**每次投递前先读，不要重新推导。**
...
pipeline.md            投后管道（笔试/面试时限 + 投递上限 + 冷却期）
                       ⚠️ 含真实投递史，已 gitignore。模板见 pipeline.example.md
```
